...
get_data = pd.read_csv('../data/wheat_name_list.csv')
e_name = get_data['e_name']
e_list = get_data['e_list'].astype(str)
...
for i in range(len(filtered_testset)):
    ...
    selected_rows = get_data.loc[(get_data['e_name'] == e_name_t) & (get_data['p_name'] == p_name_t), ['e_name', 'e_list', 'p_name', 'p_list','label']]

    e_name2.append(selected_rows['e_name'].values[0])
    e_list2.append(selected_rows['e_list'].values[0])
...
df = pd.DataFrame({'e_name': e_name2, 'e_list': e_list2, 'p_name': p_name2, 'p_list': p_list2, 'label': label2})
