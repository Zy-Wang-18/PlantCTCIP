...
for k in tqdm(range(len(file))):
    data = pd.read_csv('../' + file[k] + '.csv', encoding='utf-8')
    ...
    train_data.to_csv('../train_data_' + file[k] + '.csv', index=False)
    val_data.to_csv('../val_data_' + file[k] + '.csv', index=False)
    test_data.to_csv('../test_data_' + file[k] + '.csv', index=False)
