...
for epoch in tqdm(range(Epoch)):
    ...
    train_loss = 0.0
    train_acc_num = 0
    for i, (d_name, p_name, seq, label) in enumerate(train_loader, 0):
        seq = seq.to(device)
        ...
        loss.backward()
        clip_threshold = 1.0
        utils.clip_grad_norm_(net.parameters(), clip_threshold)
    ...
    val_acc_num = 0
    with torch.no_grad():
        for i, (d_name, p_name, seq, label) in enumerate(val_loader, 0):
            seq = seq.to(device)
            label = label.to(device)
            pred = net(seq)
            pred_lab = torch.argmax(pred, 1)
...
net.eval()
with torch.no_grad():
    for i, (d_name, p_name, seq, label) in enumerate(test_loader):
        seq = seq.to(device)
        out = net(seq).cpu()
